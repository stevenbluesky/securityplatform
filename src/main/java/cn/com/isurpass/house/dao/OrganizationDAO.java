...
public class OrganizationDAO extends BaseDAO {
	...
	public List<OrganizationPO> listOrgByType(Integer orgType){
		return (List<OrganizationPO>)getSession().createCriteria(OrganizationPO.class).add(Restrictions.eq("orgtype", orgType)).list();
	}
	...
	public List<OrganizationPO> listOrgByType(PageResult pr,Integer orgType){
		return (List<OrganizationPO>)getSession().createCriteria(OrganizationPO.class).add(Restrictions.eq("orgtype", orgType))
			    .setFirstResult(pr.getStart()).setMaxResults(pr.getRows()).list();
	}
	 ...
	 * @return
	 */
	public Integer findParentOrgId(String name) {
		return (Integer)getSession().createSQLQuery("SELECT organizationid from organization where name = ?").setString(0, name).uniqueResult();
	}
